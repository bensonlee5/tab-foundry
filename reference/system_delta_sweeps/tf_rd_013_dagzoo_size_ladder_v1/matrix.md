...
| training data surface | TabICLv2 motivates synthetic pretraining at scale but does not define this repo-local manifest contract. | Current manifest-backed prior-training corpus with data surface label `anchor_manifest_default`. | The current corpus remains the baseline while the sweep compares three shrunken shape-aware dagzoo alternatives under the same row caps. |
| dagzoo size ladder | Not applicable. | No dagzoo provenance is attached to the current-corpus anchor surface. | Each candidate row should keep one top-level `dagzoo_provenance` payload with explicit per-invocation counts so size sensitivity is reviewable rather than implicit. |
...
| training stop contract | TabICLv2 does not define this repo-local manifest trainer stop rule. | Training surface label `prior_linear_warmup_decay` with `max_steps=2500`. | The manifest-parity anchor replay clears `runtime.target_train_seconds` so later rows isolate corpus size rather than backend stop semantics. |

## Queue Summary

| Order | Delta | Family | Binary | Status | Recipe alias | Effective change | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `delta_training_manifest_parity_anchor_uncapped` | baseline_replay | yes | ready | none | Re-run the settled row-first anchor on the manifest backend with the inherited time cap cleared so the 2500-step contract is actually reached. | Run first on the CUDA machine as the canonical manifest-backed control for the reopened TF-RD-013 decision. |
| 2 | `delta_data_manifest_root_dagzoo_shape_aware_size_small` | provenance | yes | ready | none | Point training at the smallest shrunken shape-aware dagzoo manifest while keeping the three-regime config mix explicit. | Run second and compare directly against the manifest-parity anchor before spending budget on the larger ladder rungs. |
...
| 4 | `delta_data_manifest_root_dagzoo_shape_aware_size_large` | provenance | yes | ready | none | Point training at the largest shrunken shape-aware dagzoo manifest as the upper rung of the TF-RD-013 size ladder. | Run last so the size ladder resolves from manifest-parity control through small, medium, and upper-bound large corpus sizes. |
...
- Recipe alias: `none`
- Description: Re-run the settled row-first anchor on the manifest backend with the inherited time cap cleared so the 2500-step contract is actually reached.
- Rationale: Re-establish the settled row-first anchor on the manifest backend without the inherited 330-second cap so the remaining TF-RD-013 rows compare against a same-backend control.
- Hypothesis: Clearing `target_train_seconds` should let the manifest-backed anchor reach the intended 2500-step contract and become the canonical control for the dagzoo size ladder.
- Upstream delta: Not applicable; this is a repo-local manifest-parity control row for TF-RD-013.
- Anchor delta: Keep the settled row-first model, current corpus, and preprocessing surfaces fixed, but clear the inherited time cap so the manifest trainer stops only at `max_steps=2500`.
- Expected effect: Establish the canonical manifest-backed anchor control before reading any dagzoo size-rung comparison.
...
- Parameter adequacy plan:
  ...
  - Use this row as the same-backend control for rows 2-4 rather than as a new training optimization claim.
...
- Decision: `None`
- Notes:
  - The historical locked anchor artifact predates the backend-aware manifest runner; this row establishes the canonical manifest-backed control for the remaining TF-RD-013 decision.
- Follow-up run ids: `[]`
- Result card path: `outputs/staged_ladder/research/tf_rd_013_dagzoo_size_ladder_v1/delta_training_manifest_parity_anchor_uncapped/result_card.md`
...
- Anchor delta: Starting from row 1's manifest-parity anchor control, replace the current corpus with the `small` shape-aware dagzoo manifest while keeping the model, optimizer family, and stop contract fixed.
...
- Adequacy knobs to dimension explicitly:
  ...
  - manifest-contract deltas versus the manifest-parity anchor control
- Execution policy: `benchmark_full`
- Interpretation status: `pending`
- Decision: `None`
- Notes:
...
- Upstream delta: Not applicable; this is a repo-local synthetic-data corpus-size axis.
- Anchor delta: Starting from row 1's manifest-parity anchor control, replace the current corpus with the `medium` shape-aware dagzoo manifest while keeping the model, optimizer family, and stop contract fixed.
...
- Upstream delta: Not applicable; this is a repo-local synthetic-data corpus-size axis.
- Anchor delta: Starting from row 1's manifest-parity anchor control, replace the current corpus with the `large` shape-aware dagzoo manifest while keeping the model, optimizer family, and stop contract fixed.
