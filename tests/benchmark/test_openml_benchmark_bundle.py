...
def test_checked_in_binary_medium_missing_bundle_requires_explicit_missing_value_opt_in() -> None:
    bundle_path = (
        Path(__file__).resolve().parents[2]
        / "src"
        / "tab_foundry"
        / "bench"
        / "nanotabpfn_openml_binary_medium_missing_v1.json"
    )
    ...
    assert bundle["name"] == "nanotabpfn_openml_binary_medium_missing"
    assert bundle["selection"]["max_features"] == 10
    assert bundle["selection"]["max_missing_pct"] == pytest.approx(5.0)
    assert all(int(task["n_features"]) <= 10 for task in bundle["tasks"])
