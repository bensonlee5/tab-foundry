...
def build_model_from_spec(spec: ModelBuildSpec) -> nn.Module:
    ...
    if normalized_arch == SANDWICH_MODEL_ARCH:
        return TabFoundrySandwichClassifier(
            ...
            sandwich_last_stage_full_cell_refresh=bool(spec.sandwich_last_stage_full_cell_refresh),
            sandwich_column_summary_mode=str(spec.sandwich_column_summary_mode),
            sandwich_feature_encoder_kind=str(spec.sandwich_feature_encoder_kind),
            sandwich_use_class_memory=bool(spec.sandwich_use_class_memory),
