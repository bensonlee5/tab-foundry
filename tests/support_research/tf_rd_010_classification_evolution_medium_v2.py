...
SWEEP_ID = "tf_rd_010_classification_evolution_medium_v2"
EXPECTED_ROWS = [
...
def test_tf_rd_010_classification_evolution_medium_v2_is_registered() -> None:
    ...
    assert entry["parent_sweep_id"] == "tf_rd_010_classification_evolution_medium_v1"
    assert entry["status"] == "ready"
    assert entry["anchor_run_id"] == (
        "sd_tf_rd_010_classification_evolution_medium_v2_01_"
...
def test_tf_rd_010_classification_evolution_medium_v2_records_the_2500_step_successor_contract() -> None:
    ...
    assert "tf_rd_010_classification_evolution_medium_v1" in notes[0]
    ...
    assert all(
        ...
        and row["run_id"].startswith("sd_tf_rd_010_classification_evolution_medium_v2_")
        for row in rows
    )
    ...
    assert all("medium_v1" in " ".join(row["notes"]) for row in rows)
    assert all("issue `#205`" in row["next_action"] for row in rows)
    assert all(isinstance(row["benchmark_metrics"], dict) for row in rows)
    assert all("final_bpc" in row["benchmark_metrics"] for row in rows)
    ...
    assert materialized["anchor_run_id"] == sweep["anchor_run_id"]
...
def test_tf_rd_010_classification_evolution_medium_v2_matrix_records_the_pending_successor_path() -> None:
    matrix = (
    ...
    assert "Pending review for TF-RD-010 medium_v2 2500-step successor regime." in matrix
    assert "159984" in matrix
    assert "`2500` optimizer steps" in matrix
    assert "tf_rd_010_classification_evolution_medium_v1" in matrix
    assert "tf_rd_010_dagzoo_medium_control_v2" in matrix


def test_tf_rd_010_classification_evolution_medium_v2_inspection_resolves_the_2500_step_row() -> None:
