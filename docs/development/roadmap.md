...
| Expedited FFN and stability grid screens are queued | `active` | TF-RD-027 now has a config-only sweep (`tf_rd_027_grid_ffn_wd_config_v1`) for GELU 4:1, SwiGLU 8:3, and weight decay `0.1`, plus an implementation-backed follow-up sweep (`tf_rd_027_grid_stability_impl_v1`) for z-loss, logit softcap, and QK-norm under issue [#289](https://github.com/bensonlee5/tab-foundry/issues/289) | Execute the two ready FFN rows first; keep the dependent weight-decay and stability rows blocked until the config-only winner is known | `TF-RD-027`, `TF-RD-026` |
