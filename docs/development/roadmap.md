...
| 4 | TF-RD-030 | Grid MoE shared-expert follow-up | completed | Next |
...
| Sparse grid MoE is implemented but not promoted | `completed` | TF-RD-028 completed core-only SwiGLU MoE execution under issue [#295](https://github.com/bensonlee5/tab-foundry/issues/295); no row beat the TF-RD-026 row `10` anchor, raw top-2 was healthy but worse at `0.4234291888`; TF-RD-029 issue [#296](https://github.com/bensonlee5/tab-foundry/issues/296) completed normalized top-2 at `0.4218944597`, still `+0.0037177297` worse and much slower than the anchor; TF-RD-030 issue [#297](https://github.com/bensonlee5/tab-foundry/issues/297) completed shared-expert/top-2/load-balance-schedule probes, with the best row at `0.4210` (`+0.0028`) and no route collapse | Keep TF-RD-026 row `10` as the active anchor; reopen MoE only with a new hypothesis that targets quality or efficiency beyond balanced token-centric routing, shared experts, and normalized top-k | `TF-RD-028`, `TF-RD-029`, `TF-RD-030`, `TF-RD-026` |
...
- Exit decision:
  ...
  - TF-RD-030 later completed the shared-expert and auxiliary-schedule
    follow-up without promotion, so keep TF-RD-026 row `10` as the active
    anchor
...
- Completed sweep:
  ...
  - row `05`,
    ...
    optional router-temperature smoothing probe that was screened out after the
    completed rows showed healthy route balance rather than route collapse
  - all rows pin the exact TF-RD-026 row `10` corpus artifact
    `tf_rd_010_dagzoo_medium_control_curated_v6/tf_rd_010_dagzoo_medium_control_curated_v6__b54f7409b6d5`
- Completed outcomes:
  - row `01` smoke completed on Vast with W&B online run `p8h3qqpc`, benchmark
    artifacts, finite MoE telemetry, and final log loss `0.7138`
  - row `02` shared top-1 completed with W&B online run `bmura2xu`, final log
    loss `0.4210`, delta `+0.0028`, final Brier `0.2564`, final ROC AUC
    `0.8121`, train elapsed seconds `10249.6600`, and finite route health:
    load-balance loss `1.0025`, router z-loss `0.0565`, router entropy
    `1.3808`, expert fraction range `0.2365` to `0.2644`, and expert fraction
    std `0.0109`
  - row `03` shared normalized top-2 completed with W&B online run `7pzyxlea`,
    final log loss `0.4223`, delta `+0.0042`, final Brier `0.2575`, final ROC
    AUC `0.8109`, train elapsed seconds `11156.8787`, and finite balanced
    routing
  - row `04` normalized top-2 without shared expert completed with W&B online
    run `dyfqks6f`, final log loss `0.4229`, delta `+0.0047`, final Brier
    `0.2579`, final ROC AUC `0.8117`, train elapsed seconds `10061.5358`, and
    finite balanced routing; it used less reserved GPU memory than shared
    top-2, but its W&B effective-coefficient telemetry stayed at `0.01`, so
    the load-balance decay interpretation remains confounded
  - row `05` was screened out because route health was already balanced and
    finite across the completed rows, so router-temperature smoothing no longer
    targeted the observed gap
- Exit decision:
  - close TF-RD-030 without promotion
  - keep TF-RD-026 row `10` as the active anchor because every MoE candidate
    remained worse on final log loss and slower than the dense anchor, despite
    healthy token-centric routing and finite z-loss/load-balance telemetry
