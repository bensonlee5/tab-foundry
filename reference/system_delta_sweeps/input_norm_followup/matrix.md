...
| input normalization | The bridge baseline still uses `train_zscore_clip`. | Keep `train_zscore_clip` on the locked batch64 sqrt-scaled hybrid-diagnostic surface. | Rows 1-5 showed the normalized variants were tied on batch32, row 9 only tied row 7, and the later no-norm follow-up underperformed, so clipping stays in the locked hybrid-diagnostic anchor by default. |
| batch-size scaling | Exact-prior training previously treated batch size as a hidden function argument rather than a tracked surface knob. | Use prior-dump batch size 64 with sqrt LR scaling relative to the reference batch size of 32. | Batch-size rows were the only source of clear lift in this sweep; row 7 is the locked systems surface for this hybrid-diagnostic lane. |

## Queue Summary

| Order | Delta | Family | Binary | Status | Recipe alias | Effective change | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `dpnb_input_norm_anchor_replay` | baseline_replay | yes | completed | none | Replay the locked hybrid-diagnostic baseline exactly on this machine before comparing normalization or batch-size variants. | Treat this run as the batch32 reference only; row 7 (`dpnb_input_norm_anchor_replay_batch64_sqrt`) is the locked hybrid-diagnostic anchor. |
| 2 | `dpnb_input_norm_zscore` | input_normalization | yes | completed | none | Remove external clipping and use plain train-only z-score normalization on the locked hybrid-diagnostic baseline. | Interpret rows 2-5 as a normalization-family wash on the weaker batch32 surface; keep row 7 as the locked hybrid-diagnostic anchor. |
| 3 | `dpnb_input_norm_winsorize_zscore` | input_normalization | yes | completed | none | Winsorize at the 1st and 99th train percentiles before z-scoring on the locked hybrid-diagnostic baseline. | Keep this as bounded outlier-defense evidence only; row 7 stays the locked hybrid-diagnostic anchor because the batch64 surface moved the metric while rows 1-3 did not. |
| 4 | `dpnb_input_norm_zscore_tanh` | input_normalization | yes | completed | none | Apply train-only z-score normalization followed by a smooth tanh tail squash on the locked hybrid-diagnostic baseline. | Treat this as the smooth-tail comparator only; row 9 later tied the locked winner, so row 7 stays the locked hybrid-diagnostic anchor because it reaches the same result with less normalization. |
| 5 | `dpnb_input_norm_robust_tanh` | input_normalization | yes | completed | none | Apply robust median/IQR scaling followed by a smooth tanh tail squash on the locked hybrid-diagnostic baseline. | Keep this as negative evidence for the robust smooth-tail branch; it did not separate from the batch32 baseline and did not justify replacing row 7. |
| 6 | `dpnb_input_norm_anchor_replay_batch16_sqrt` | batch_size | yes | completed | none | Replay the locked hybrid-diagnostic baseline at prior-dump batch size 16 with sqrt LR scaling. | Keep this as negative batch-size evidence only; the smaller batch is not competitive with the locked row-7 hybrid-diagnostic anchor. |
| 7 | `dpnb_input_norm_anchor_replay_batch64_sqrt` | batch_size | yes | completed | none | Replay the locked hybrid-diagnostic baseline at prior-dump batch size 64 with sqrt LR scaling. | Use this run as the locked hybrid-diagnostic anchor for future binary_md follow-ups; benchmark-facing promotion claims still need the `cls_benchmark_staged` architecture-screen surface. |
...
| 9 | `dpnb_input_norm_zscore_tanh_batch64_sqrt` | batch_size | yes | completed | none | Combine the favored smooth-tail zscore_tanh normalization with prior-dump batch size 64 and sqrt LR scaling. | Keep this as tie evidence only; row 7 remains the locked hybrid-diagnostic anchor because it matched this result with less invasive preprocessing. |
...
- Rationale: Reproduce the locked hybrid-diagnostic baseline locally before interpreting any normalization or batch-size movement.
- Hypothesis: The row-cls hybrid-diagnostic winner should replay cleanly on this machine and provide the correct comparator for the rest of the queue.
- Upstream delta: Not applicable; this is a repo-local metadata-only hybrid-diagnostic anchor replay.
- Anchor delta: Replays `dpnb_row_cls_cls2_linear_warmup_decay` exactly, keeping `train_zscore_clip`, batch size 32, and the locked linear-warmup-decay schedule.
...
- Parameter adequacy plan:
  ...
  - If a later row wins cleanly, preserve this replay as evidence rather than forcing it to remain the locked hybrid-diagnostic anchor.
...
- Decision: `defer`
- Notes:
  - This row is required because the locked hybrid-diagnostic baseline artifacts were produced on another machine.
  ...
  - CUDA rerun established the same-machine batch32 reference, but row 7 later displaced it as the locked hybrid-diagnostic anchor.
...
- Notes:
  ...
  - CUDA rerun recorded the canonical benchmark comparison, but the row remains deferred pending full sweep interpretation.
  - Robust smooth-tail preprocessing produced no measurable upside on the locked bundle, so it stays below the locked clipped batch64 hybrid-diagnostic surface.
- Follow-up run ids: `[]`
...
- Notes:
  ...
  - CUDA rerun recorded the canonical benchmark comparison, but the row remains deferred pending full sweep interpretation.
  - Batch size 16 hurt final ROC AUC relative to both the batch32 reference and the locked batch64 hybrid-diagnostic surface.
- Follow-up run ids: `[]`
...
- Notes:
  ...
  - CUDA rerun recorded the canonical benchmark comparison, but the row remains deferred pending full sweep interpretation.
  - This row stays locked as the hybrid-diagnostic anchor because it matched row 9 while keeping `train_zscore_clip`, which is the less invasive preprocessing choice on this implementation.
