...
- Locked `dpnb_row_cls_cls2_linear_warmup_decay` as the new hybrid-diagnostic default for `cls_benchmark_staged_prior`.
...
- This default is formalized as sweep metadata only on this machine; the canonical benchmark anchor remains deferred until the remote artifacts are synced and registered locally.
