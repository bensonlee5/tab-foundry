...
def test_tf_rd_010_classification_evolution_medium_v5_is_registered_as_ready_successor() -> None:
...
def test_tf_rd_010_classification_evolution_medium_v5_records_the_sorted_control_followup() -> None:
    ...
    assert any("large_v2" in note for note in notes)
    ...
    assert "sorted-order code path" in row["next_action"]
    ...
    assert materialized_row["status"] == "ready"
...
def test_tf_rd_010_classification_evolution_medium_v5_resolved_queue_and_matrix_capture_the_followup() -> None:
    ...
    assert [row["status"] for row in resolved["rows"]] == ["ready"]
    assert [row["run_id"] for row in resolved["rows"]] == [None]
    assert [row["decision"] for row in resolved["rows"]] == [None]
    ...
    assert "single-row follow-up" in matrix
