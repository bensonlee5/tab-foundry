...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `delta_data_manifest_root_tf_rd_010_dagzoo_medium_control` | provenance | no | ready | Point training at the TF-RD-010 control corpus: a balanced 144-task DAGZoo grid capped at `<=1024` rows per dataset, with row totals `128/256/512/1024`, feature counts `6/10/14/20`, and class coverage `2..10`. | Rerun this row as the trusted medium anchor after issue `#204` lands, then use it as the comparison point for the remaining medium rows under issue `#205`. |
