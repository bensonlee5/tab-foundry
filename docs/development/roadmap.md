...
| Expedited FFN and stability grid screens are queued | `active` | TF-RD-027 config-only FFN rows completed under issue [#289](https://github.com/bensonlee5/tab-foundry/issues/289): GELU 4:1 reached final log loss `0.4254928909`, SwiGLU 8:3 reached `0.4261833356`, and both lost to the TF-RD-026 row `10` anchor `0.4181767299`; the implementation-backed follow-up sweep (`tf_rd_027_grid_stability_impl_v1`) remains queued for z-loss, logit softcap, and QK-norm | Keep the current anchor FFN for now; run the dependent weight-decay row only after explicitly carrying the anchor FFN into the row, then copy the config-only winner/current anchor surface into stability follow-ups | `TF-RD-027`, `TF-RD-026` |
...
- Config-only sweep:
  - `tf_rd_027_grid_ffn_wd_config_v1` completed the two ready FFN rows:
