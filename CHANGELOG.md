...
- User-facing note: corpus manifest characteristics now treat upstream
  ...
  floor moves to `tab-realdata-hub>=0.1.8` for the fast large-corpus manifest
  builder.
...
- User-facing note: completed system-delta rows now synchronize back into the
  live queue state via the benchmark registry, preventing concurrent row runs
  from reverting already-finished rows to `ready` and preserving the row
  objective metric during registry-backed recovery.
...
- Internal note: refactored `tabfoundry_sandwich` into shared and family-local
  helper modules so the classification and `cell_bpc` paths are easier to
  maintain and test while preserving the existing public API, checkpoint keys,
...
- Internal architecture note: hook and audit tooling now resolve interpreter
  roots worktree-first with primary-checkout fallback, the exact-prior
  training surface now lives under `tab_foundry.training`, and benchmark-run
  registry reads now flow through the canonical top-level helper.
