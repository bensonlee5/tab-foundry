...
- For fast agent triage, prefer the existing narrow inspection surfaces before broad greps or full-suite runs: `./scripts/dev verify paths <PATH...>`, `uv run tab-foundry dev resolve-config ...`, `uv run tab-foundry dev forward-check ...`, `uv run tab-foundry dev diff-config ...`, `uv run tab-foundry dev export-check --checkpoint ...`, `uv run tab-foundry data manifest-inspect --manifest ...`, `uv run tab-foundry dev run-inspect --run-dir ...`, and `uv run tab-foundry research sweep inspect|diff ...`.
