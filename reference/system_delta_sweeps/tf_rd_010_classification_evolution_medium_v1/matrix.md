...
| validation surface | Not applicable. | Hub-backed medium multiclass manifest at `data/manifests/bench/nanotabpfn_openml_classification_medium_v1/manifest.parquet`. | Keep the no-missing medium rung fixed while the synthetic training front changes across rows. |

## Queue Summary

| Order | Delta | Family | Binary | Status | Recipe alias | Effective change | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `delta_data_manifest_root_tf_rd_010_dagzoo_medium_control` | provenance | no | completed | none | Point training at the TF-RD-010 dagzoo multiclass control corpus while the evolved sandwich benchmark contract is defined against hub-owned validation manifests. | Completed as the locked medium control anchor; keep this row as the fixed TF-RD-010 reference and move any improvement attempts to TF-RD-021 or later higher-budget follow-up instead of reopening baseline definition. |
...
- Anchor delta: Use the evolved FiLM plus 3-summary-token sandwich contract and train on `tf_rd_010_dagzoo_medium_control_v1` while validating on the hub-owned medium multiclass manifest.
