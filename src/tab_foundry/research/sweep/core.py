...
def _run_list_sweeps(args: argparse.Namespace) -> int:
    for sweep_info in list_sweeps(index_path=_index_path(args)):
...
def _run_show_active(args: argparse.Namespace) -> int:
    index = load_system_delta_index(_index_path(args))
...
def _load_queue_from_args(args: argparse.Namespace) -> dict[str, object]:
    ...
    return load_system_delta_queue(
...
def _run_sweep_render(args: argparse.Namespace) -> int:
    ...
    active_sweep_id = ensure_non_empty_string(
        load_system_delta_index(_index_path(args)).get("active_sweep_id"),
...
def _run_sweep_validate(args: argparse.Namespace) -> int:
    queue = _load_queue_from_args(args)
    issues = validate_system_delta_queue(queue, registry_path=_registry_path(args))
    if not issues:
