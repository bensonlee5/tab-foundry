...
from tab_foundry.research.sweep.materialize import (
...
def test_materialize_sweep_corpora_materializes_unique_queue_corpus_refs(
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: {
            ...
            "rows": [
                {"order": 2, "delta_id": "delta_b", "data": {"corpus_ref": "global_recipe"}},
                {"order": 1, "delta_id": "delta_a", "data": {"corpus_ref": "local_recipe"}},
                {
                    "order": 3,
                    "delta_id": "delta_c",
                    "data": {"corpus_ref": "local_recipe/local_recipe__cached"},
                },
                {"order": 4, "delta_id": "delta_d", "data": {}},
            ],
        },
    )
...
def test_create_sweep_supports_missing_permitting_bundle_anchor_surface(tmp_path: Path) -> None:
    ...
    assert created_sweep["benchmark_manifest_path"] == "data/manifests/bench/openml_classification_large_v1/manifest.parquet"
...
def test_create_sweep_bootstraps_tf_rd_018_corrected_baseline_on_corpus_surface(
    tmp_path: Path,
) -> None:
    reference_root, sweeps_root = _copy_reference_workspace(tmp_path)

    _ = create_sweep(
        sweep_id="tf_rd_018_baseline_clone",
        anchor_run_id="sd_tf_rd_020_harder_dagzoo_ladder_v1_06_delta_data_manifest_root_tf_rd_020_shift_noise_drift_v2",
        parent_sweep_id="tf_rd_018_optimizer_family_v1",
        complexity_level="binary_md",
        benchmark_manifest_path="data/manifests/bench/openml_classification_medium_v1/manifest.parquet",
