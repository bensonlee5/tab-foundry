...
def test_manifest_dataset_threads_categorical_feature_state_from_metadata(tmp_path: Path) -> None:
    shard_dir = tmp_path / "run" / "shard_00000"
    dataset = {
        "dataset_index": 0,
...
def test_manifest_dataset_threads_categorical_feature_state_for_staged_tasks(tmp_path: Path) -> None:
    ...
    assert sample.feature_state.categorical_cardinalities.tolist() == [0, 2]
    assert torch.allclose(sample.x_train, torch.tensor([[1.0, 0.0], [3.0, 1.0], [5.0, 0.0]]))
    assert torch.allclose(sample.x_test, torch.tensor([[7.0, 2.0], [9.0, 1.0]]))
