...
def resolve_data_surface(data_cfg: Mapping[str, Any] | None) -> DataSurfaceConfig:
    ...
    source = str(overrides.get("source", cfg.get("source", "manifest"))).strip().lower()
    raw_manifest_path = overrides.get("manifest_path", cfg.get("manifest_path"))
    manifest_path = None
    if raw_manifest_path is not None:
        manifest_path = Path(str(raw_manifest_path)).expanduser().resolve()
    filter_policy_raw = overrides.get("filter_policy", cfg.get("filter_policy"))
    filter_policy = None if filter_policy_raw is None else str(filter_policy_raw).strip()
    dagzoo_provenance_raw = overrides.get(
        "dagzoo_provenance",
        cfg.get("dagzoo_provenance"),
    )
    dagzoo_provenance = (
        None
