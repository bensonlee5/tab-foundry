...
ExactSignature = tuple[int, int, int, int | None]


def _signature_family(signature: ExactSignature) -> SignatureFamily:
...
class SignatureFamilyCompileDispatcher:
    """Compile one callable per exact task signature inside admitted shape families."""
    ...
    def __init__(
        ...
        *,
        ...
        self._compiled_by_signature: dict[ExactSignature, Any] = {}
        self._admitted_families: dict[SignatureFamily, None] = {}
        self._family_hits = 0
    ...
    def __call__(self, batch: TaskBatch):
        ...
        family_admitted = family in self._admitted_families
        if not family_admitted and len(self._admitted_families) >= self._max_families:
        ...
        self._admitted_families.setdefault(family, None)
        return compiled(batch)

    def summary(self) -> dict[str, Any]:
        compiled_families = sorted(_signature_family_text(family) for family in self._admitted_families)
        return {
            "family_cache_hits": int(self._family_hits),
            "family_cache_misses": int(self._family_misses),
            "compiled_family_count": int(len(self._admitted_families)),
